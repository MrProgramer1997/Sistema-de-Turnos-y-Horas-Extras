...
// Payroll-only transport 7.17. Other modules are not modified. Never substitutes a public key for a
...
function esJwtInvalido(error) {
  if (codigo(error) === '42501') return false;
  return ['PGRST301','PGRST302','PGRST303'].includes(codigo(error)) ||
    Number(error?.status) === 401 || /jwt.*(expired|invalid)/i.test(error?.message || '');
}
function esPermisoDenegado(error) {
  return codigo(error) === '42501' || Number(error?.status) === 403 ||
    /permission denied|^(perfil no autorizado|usuario no autorizado|usuario sin perfil activo|sin autorizacion|sesion.*no autorizada)/i.test(error?.message || '');
}
function esTokenInvalido(error) {
  return Number(error?.status) === 401 ||
    /AuthSessionMissing|AuthInvalidJwt|session_not_found|refresh_token_not_found|refresh_token_already_used|bad_jwt|user_banned|user_not_found/i.test(`${error?.name || ''} ${error?.code || ''}`);
}
...
export function crearControlSesion(client, { localSession = leerSesionVisual, timeout = 12000, queryTimeout = 40000, clock = Date.now } = {}) {
  ...
  async function verificar() {
    ...
    if (result.error) {
      ...
      if (result.error) {
        if (esTokenInvalido(result.error)) throw faltaSesion();
        throw new ErrorSesion('AUTH_NETWORK', 'No se pudo verificar la cuenta tras renovar. Reintenta la consulta; no se han eliminado tus datos.', result.error);
      }
    }
  ...
  async function query(makeBuilder, { signal, range, read = true, name = "consulta protegida" } = {}) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      let agotada = false;
      const timer = setTimeout(() => { agotada = true; abort(); }, queryTimeout);
      let response;
      try { response = await builder.abortSignal(controller.signal); }
      finally { clearTimeout(timer); signal?.removeEventListener('abort', abort); }
      if (signal?.aborted) throw new DOMException('Consulta cancelada','AbortError');
      if (agotada) {
        const error = new Error('La consulta excedio el tiempo de espera. No se repetira ninguna escritura.');
        error.code = 'QUERY_TIMEOUT'; error.fuenteNomina = name; throw error;
      }
      if (controller.signal.aborted) throw new DOMException('Consulta cancelada','AbortError');
      if (version !== generation) throw faltaSesion();
      // A database error is not an expired login. Preserve the SQL error and
      // never refresh or replay a write because it timed out or was forbidden.
      if (response.error && esPermisoDenegado(response.error)) {
        throw new ErrorSesion('AUTH_FORBIDDEN', `La cuenta esta verificada, pero no tiene permiso para ${name}. Sistemas debe revisar ese permiso.`, response.error);
      }
      if (response.error && !esJwtInvalido(response.error)) return response;
      const latest = await actual();
      if (latest.user.id !== valid.user.id) throw new ErrorSesion('AUTH_IDENTITY', 'La cuenta cambio durante la consulta. Vuelve a ingresar.');
      ...
      throw new ErrorSesion('AUTH_REQUIRED', 'El servidor no acepto la sesion segura. Vuelve a ingresar y consulta lo guardado antes de repetir una operacion.', response.error);
...
// Only confirmed missing/mismatched identity clears payroll data. A timeout,
// missing permission, unavailable SDK or network failure never implies logout.
export const esErrorAcceso = error => error instanceof ErrorSesion && ['AUTH_REQUIRED','AUTH_IDENTITY'].includes(error.code);
