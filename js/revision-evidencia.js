...
export function crearVisorRevision({rpc,onChanged=()=>{},onError=()=>{},onBusy=()=>{},totalEntreMarcas=false,canWrite=()=>true}){
 ...
 return async function mostrar(x,{nocturno=false}={}){
  ...
  try{
   ...
   if(nocturno||requiereRevisionNocturna(x)){
    ...
    function habilitar(){const h=Number(horas.value);btn.disabled=ocupado||!canWrite()||!check.checked;aprobar.disabled=btn.disabled||!horas.value||!Number.isFinite(h)||h<=0||h>Number(p.horas_candidatas);}
    ...
    async function guardar(decidir){
     ...
     try{
      ...
      await onChanged(saved,e);status.textContent=decidir?'Aprobacion confirmada y registrada en auditoria.':'Candidato actualizado; no se aprobó ningún pago.';btn.hidden=aprobar.hidden=true;mot.disabled=check.disabled=horas.disabled=true;
