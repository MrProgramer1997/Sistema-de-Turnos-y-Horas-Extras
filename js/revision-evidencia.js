...
export function crearVisorRevision({rpc,onChanged=()=>{},totalEntreMarcas=false,canWrite=()=>true}){
 ...
 return async function mostrar(x,{nocturno=false}={}){
  ...
  try{
   ...
   if(nocturno||requiereRevisionNocturna(x)){
    ...
    function habilitar(){const h=Number(horas.value);btn.disabled=ocupado||!canWrite()||!check.checked||mot.value.trim().length<10;aprobar.disabled=btn.disabled||!horas.value||!Number.isFinite(h)||h<=0||h>Number(p.horas_candidatas);}
    mot.oninput=habilitar;check.onchange=habilitar;horas.oninput=habilitar;
    async function guardar(decidir){
     if(ocupado||!canWrite()||(decidir?aprobar:btn).disabled)return;
     ocupado=true;habilitar();dlg.querySelector('[data-close]').disabled=true;status.textContent=decidir?'Registrando aprobación y auditoría...':'Actualizando candidato, sin aprobar...';
     try{
      ...
      if(!saved?.id||(decidir&&saved.estado!=='aprobado'))throw new Error('No se confirmó el estado guardado');
      await onChanged(saved,e);status.textContent=decidir?'Aprobación confirmada; horas y comentario auditados.':'Candidato actualizado; no se aprobó ningún pago.';btn.hidden=aprobar.hidden=true;mot.disabled=check.disabled=horas.disabled=true;
     }catch(err){status.textContent='No se confirmó la operación: '+(err.message||err)+'. Cierra y actualiza antes de reintentar. No se repite automáticamente.';btn.hidden=aprobar.hidden=true;}
     finally{ocupado=false;dlg.querySelector('[data-close]').disabled=false;}
    }
    btn.onclick=()=>guardar(false);aprobar.onclick=()=>guardar(true);
