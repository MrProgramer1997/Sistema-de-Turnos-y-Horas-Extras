...
export function calculadasCeldaRevision(x,m=modeloRevision(x)){
 ...
 if(requiereRevisionNocturna(x)){
  ...
  return `<strong>${x.detalle?.calculo_pendiente?'Por verificar':textoHoras728(x.horas_calculadas)+' guardadas'}</strong><small class="rev-small">${bruto===null?'Falta un par válido del punto.':'El importe se verifica con las marcas del punto al abrir la revisión.'}</small>`;
 }
 return `<strong>${textoHoras728(x.horas_calculadas)}</strong>`;
}
...
export function crearVisorRevision({rpc,onChanged=()=>{},onError=()=>{},onBusy=()=>{},totalEntreMarcas=false,canWrite=()=>true,confirmacionExplicita=true,canManual=()=>false,onManual=()=>{}}){
 ...
 return async function mostrar(x,{nocturno=false}={}){
  ...
  try{
   ...
   if(nocturno||requiereRevisionNocturna(x)){
    ...
    if(!p.calculable&&canManual()){version++;dlg.close();void onManual(x);return;}
    ...
    function habilitar(){const h=horasTextoONaN728(horas.value);btn.disabled=ocupado||!canWrite()||(confirmacionExplicita&&!check?.checked);aprobar.disabled=btn.disabled||!horas.value||!Number.isFinite(h)||h<=0||h>Number(p.horas_candidatas);}
    ...
    async function guardar(decidir){
     ...
     try{
      const args={p_revision_id:p.revision_id,p_version:p.updated_at,p_huella:p.huella,p_motivo:mot.value.trim()};if(decidir)args.p_horas=horasDesdeTexto728(horas.value);
