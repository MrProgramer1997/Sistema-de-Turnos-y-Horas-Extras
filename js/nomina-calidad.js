...
export function filaHorario(x){return (x.jornada_actual?.documental_711?x.jornada_actual:null)||x.evidencia_revision?.horario_documental||x.evidencia_revision?.horario||x.jornada_actual||x;}
export function estadoHorario(x){const p=filaHorario(x);if(p.documental_711?p.conflicto_programacion:(x.evidencia_revision?.conflicto_horario||x.conflicto_programacion))return 'conflicto';if((!p.documental_711&&x.evidencia_revision?.horario)||p.programacion_tipo==='confirmada'||(!p.documental_711&&x.programacion_tipo==='confirmada'))return 'guardado';const t=s(p.programacion_tipo).toLowerCase();if(t==='inferida_alta')return 'alta';if(t==='inferida_media')return 'media';if(t==='inferida_ambigua'||p.estado_comparacion==='turno_ambiguo')return 'por_confirmar';return 'sin_horario';}
export function diagnosticoHorario(x){const p=filaHorario(x),tipo=estadoHorario(x);if(p.documental_711)return p.documental_711.tipo==='documental_fija'?('Plantilla estipulada por Bienestar, vigente desde '+p.documental_711.desde+'. '+p.documental_711.fuente):p.diagnostico_turno||p.documental_711.plan?.motivo||'Horario documental por revisar';if(tipo==='guardado')return p.tipo_registro==='novedad'||p.novedad_codigo?'Novedad guardada; no equivale a horas de trabajo':'Horario guardado para esta persona y fecha; prevalece sobre la inferencia';if(tipo==='conflicto')return 'Fuentes de programacion incompatibles; revisar antes de comparar';return s(p.diagnostico_turno||x.diagnostico_turno)||((n(x.total_marcaciones)||0)===1?'Una marcacion: se conserva la presencia; falta completar evidencia':tipo==='sin_horario'?'No hay horario asignado ni una plantilla utilizable para esta fecha':'Horario sugerido por las marcas; no confirma asignacion ni autoriza extras');}
