...
export function horasDomingo(valor){
 const m=text(valor).match(/^(\d{1,2}):([0-5]\d)$/);
 if(!m)throw new Error('Escribe horas:minutos, por ejemplo 04:17.');
 const minutos=+m[1]*60+ +m[2];
 if(minutos<=0||minutos>1440)throw new Error('El tiempo debe ser mayor que cero y no superior a 24:00.');
