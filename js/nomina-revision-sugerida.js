...
export function crearRevisionSugerida719({rpc,canWrite,onBusy,onSaved,onError}){
 ...
 function paint(){
  ...
  body.innerHTML=`<form novalidate><h3>${esc(x.empleado||r.evidencia.empleado)}</h3>
   ...
   ${action==='aprobar'?`<label for="ns719Horas">Horas de este concepto (decimal; maximo ${esc(c?.horas_maximas??'por verificar')})</label><input class="form-control" type="number" step="0.01" min="0.01" id="ns719Horas" value="${esc(c?.horas_maximas??'')}" ${canApprove?'':'disabled'}>`:''}
   ...
   <details class="nd-evidencia"><summary>Ver marcaciones y otros conceptos</summary><p>${esc(r.criterio)}</p><p>${(r.evidencia.avisos||[]).map(esc).join(" ")}</p><p>${r.conceptos.map(z=>`${esc(z.codigo)} ${esc(z.nombre)}: ${esc(z.horas_maximas)} h de referencia`).join('<br>')||'Sin intervalo calculable con el turno seleccionado.'}</p>
 ...
 async function save(ev){
  ...
  try{
   ...
   if(!ayb)args.p_confirmado=confirmed;
   const result=await call(ayb?'resolver_ayb_nomina_v721':'resolver_sugerencia_nomina_v719',args,false);
