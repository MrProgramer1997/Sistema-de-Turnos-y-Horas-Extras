...
import { intervaloDiario, duracionMarcaciones, fechaHoraMarcacion, descansoReferencia, estadoMarcacion, coincideMarcacion, columnasIntervalo, hojaIntervalos, CRITERIO_INTERVALO } from './nomina-marcaciones.js?v=713';
...
import { duracionRevision, nocturnoPosteriorRevision, calculadasCeldaRevision, modeloRevision, comparacionCelda, programacionCelda, marcadoCelda, totalCelda, advertenciaNocturna, requiereRevisionNocturna, crearVisorRevision, columnasRevisionExcel } from "./revision-evidencia.js?v=713";
...
import { cargarFuentesNomina, leerPaginas, diasEntre, recalcularPorDias } from "./nomina-carga.js?v=7-10";
...
async function cargar(){
  ...
  try{
    ...
    const coberturaDomingos=await incorporarDomingos(rpcConSesion,desde,hasta,{signal});
    ...
    avisoCarga("Consultando horarios documentados y evidencia de semanas completas...");
    const nuevaDocumentacion=await cargarDocumentados(lecturaRpc,{desde,hasta,signal});
    const nuevasJornadas=nuevaDocumentacion.disponible?superponerDocumentados(jornadasPrevias,nuevaDocumentacion.resuelto):jornadasPrevias;
    ...
    const nuevasEvidencias=nuevaDocumentacion.disponible?fusionarEvidenciasDocumentales(evidenciasPrevias,nuevaDocumentacion.resuelto):evidenciasPrevias;
...
function filtrados(){const a=$("heArea").value,e=$("heEstado").value,b=texto($("heBuscar").value).toLowerCase();return base.filter(x=>coincideDia(x)&&dentroAlcance(x)&&coincideAreaConsulta(x,a)&&(!e||estado(x)===e)&&(!b||`${empleado(x)} ${x.cedula||""} ${codigoErp(x)} ${concepto(x)}`.toLowerCase().includes(b))).sort(ordenCronologicoRevision)}
...
function renderTabla(rows){
  ...
  $('heHead').innerHTML='<tr><th>Empleado</th><th>Fecha</th><th>Programado</th><th>Marcado</th><th>Ingreso</th><th>Salida</th><th>Total entre marcas</th><th>Concepto</th><th>Calculadas</th><th>Aprobadas</th><th>Estado y observación</th><th>Acciones</th></tr>';
  ...
  $('heBody').innerHTML=visibles.map(x=>{
    const m=modeloRevision(x),riesgo=requiereRevisionNocturna(x),desfase=riesgo?null:controlExtraVigente(x,m),abierta=puedeDecidir()&&x.permite_revision&&!['aprobado','rechazado'].includes(estado(x));
    ...
    const acciones=abierta?`<div class="rev-actions">${domingo?boton('validarDomingo','Revisar y aprobar','outline-primary'):desfase?`<button class="btn btn-outline-primary btn-sm" onclick="window.heVerRevision('${html(x.revision_id)}')">Revisar horario y calculo</button>`:riesgo?`<button class="btn btn-outline-primary btn-sm" onclick="window.heVerRevision('${html(x.revision_id)}',true)">Revisar y aprobar nocturno</button>`:boton('aprobar','Aprobar '+horasCalculadas(x).toFixed(2)+' h','success')+boton('ajustar','Ajustar','outline-primary')}${boton('observar','Observar','outline-warning')}${boton('rechazar','Rechazar','outline-danger')}</div>`:'<span class="text-muted small">'+(x.permite_revision?'Actualiza la consulta':'Cerrado')+'</span>';
    return `<tr><td><strong>${html(empleado(x))}</strong><small class="rev-small">${html(codigoErp(x))} · ${html(x.cedula)}<br>${html(area(x))}</small></td><td>${fechaDiaRevision(x.fecha)}${fechaHtml(x,heCalendario)}</td><td>${programacionCelda(m)}${diagnosticoHorarioHtml(x)}</td><td>${marcadoCelda(m)}<button class="rev-detail-button" onclick="window.heVerRevision('${html(x.revision_id)}')">Ver todas las marcas</button></td><td>${comparacionCelda(m,'entrada')}</td><td>${comparacionCelda(m,'salida')}</td><td>${celdaIntervalo(intervaloDiario(x,m))}</td><td><strong>${html(concepto(x))}</strong><div class="rev-small">${html(x.concepto_nombre)}</div></td><td>${domingo?`<strong class="dom712-pendiente">${textoCalculoDomingo(x)}</strong><div class="rev-small">Validar trabajo y pausas. No significa cero trabajado.</div>`:calculadasCeldaRevision(x,m)}${advertenciaNocturna(x)}${desfase?`<div class="he-candidato-desfasado">${html(desfase)}</div>`:""}${controlConceptoHtml(x,heCalendario,m)}${texto(x.origen_calculo)==='dominical_marcaciones_v1'?'<div class="rev-small">Dominical estimado; revisar antes de aprobar</div>':''}</td><td>${horasAprobadas(x)===null?'—':horasAprobadas(x).toFixed(2)+' h'}</td><td>${badge(estado(x))}<div class="rev-small">${html(x.observacion||'Sin observación registrada')}</div></td><td>${acciones}</td></tr>`;
...
function celdaTrabajo(x){
  return `${celdaIntervalo(intervaloDiario(x))}<button class="btn btn-link btn-sm p-0" data-detalle-dia="${html(claveDia(x))}">Ver detalle</button>`;
}
function abrirDetalleDia(clave){
 const x=jornadas.find(r=>claveDia(r)===clave);if(!x||!datosUtilizables())return;
 mostrarRevisionEvidencia(x,{nocturno:false});
}
...
async function validarDomingoRevision(x){
  ...
  try{
    const decision=await pedirValidacionDomingo(x,modeloRevision(x));
    if(!decision||carga!==heCargaId)return;
    avisoCarga("Guardando las horas verificadas y su justificacion...");
    ...
    avisoCarga('Aprobacion confirmada por Supabase; horas y motivo registrados en auditoria.','success');
  }catch(e){
    if(carga!==heCargaId)return;
    heLecturaValida=false;
    if(esErrorAcceso(e)){limpiarNominaPorSesion();mostrarErrorAcceso($("heIntegridad"),e,cargar);}
    avisoCarga(`No se confirmo la decision: ${e.message||e}. Actualiza antes de reintentar; no se repite automaticamente.`,'warning');
...
function descargarRevision(){
  ...
  const rows=filtrados(),js=jornadasFiltradas({ignorarEstadoMarcacion:vista!=="jornadas"}),wb=XLSX.utils.book_new();
