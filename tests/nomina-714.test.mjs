...
test('Cambiar cantidad exige justificacion y accion ajustar',()=>{assert.throws(()=>decide({horas:'00:20'}));assert.equal(decide({horas:'00:20',motivo:'Tiempo efectivo verificado'}).accion,'ajustar');});
...
test('Rechazo no inventa horas y exige motivo',()=>{assert.throws(()=>decide({accion:'rechazar',motivo:''}));assert.deepEqual(decide({accion:'rechazar',motivo:'No autorizado'}),{accion:'rechazar',horas:null,motivo:'No autorizado'});});
test('Domingo exige ingreso manual justificado',()=>{assert.throws(()=>decide({accion:'validarDomingo'}));assert.equal(decide({accion:'validarDomingo',horas:'08:20',motivo:'Verificado el domingo y descontado almuerzo'}).horas,8.33);});
